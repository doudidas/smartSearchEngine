...
public class UserResource {
    ...
    User getUserByEmail(String email) {
        users = userCollection.find(eq("email", email)).map(UserResource::docToUser).into(new ArrayList<User>());
        return (users.size() == 1) ? users.get(0) : null;
    }

    User getUserById(String id) {
        users = userCollection.find(eq("_id", new ObjectId(id))).map(UserResource::docToUser).into(new ArrayList<User>());
        return (users.size() == 1) ? users.get(0) : null;
    }
